...
MODULE_PATH = Path(__file__).with_name("score_full_ocr_candidate_v2.py")
SPEC = importlib.util.spec_from_file_location("score_full_ocr_candidate_v2", MODULE_PATH)
...
def test_condition_caption_has_phase_semantics_and_unknown_roles_fail() -> None:
    assert scorer._canonical_role("condition_label") == "phaseheading"
    assert "other" in scorer.RUNTIME_ROLES
...
def test_condition_caption_scores_phase_heading_without_changing_geometry_or_text():
    truth = _truth("1", scorer.Box(0, 0, 10, 10), "A", "condition_label")
    caption = _prediction("p", scorer.Box(0, 0, 10, 10), "A", "phaseheading")
    correct = scorer._score_split((truth,), {"a" * 64: (caption,)})
    wrong = scorer._score_split((truth,), {"a" * 64: (replace(caption, role="other"),)})
    assert correct["role_accuracy"] == 1.0 and wrong["role_accuracy"] == 0.0
    for key in ("geometry_matched_region_count", "recognition_exact_count", "character_error_count"):
        assert correct[key] == wrong[key]
