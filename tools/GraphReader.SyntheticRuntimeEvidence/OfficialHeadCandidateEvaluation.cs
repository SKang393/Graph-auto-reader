...
internal static class OfficialHeadCandidateEvaluation
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        bool headerContext = args[0] == HeaderContextCommand;
        ...
        string composition = headerContext
            ...
            : insidePlot ? ProductionOcrAdapter.InsidePlotCandidateCompositionVersion
        ...
        try
        {
            ...
            ProductionOcrAdapter adapter = await (participantLane
                ...
                : ProductionOcrAdapter.CreateForFrozenDbHeadCandidateEvaluationAsync(
                    ...
                    headerLayoutContext: headerContext))
            ...
            foreach (EvaluationPanel panel in panels)
            {
                ...
                try
                {
                    ...
                    else if (insidePlot)
                    {
                        ...
                        if (pixelBounds)
                        {
                            ...
                            IReadOnlyList<OcrDetectedRegion> refined = OriginalPixelTextRegionRefiner.Refine(
                                original, groups.Select(static group => group.Region).ToArray(), cancellationToken);
                            groups = groups.Select((group, index) => group with { Region = refined[index] }).ToArray();
                        }
                        effective = groups.Select(static group => group.Region).ToArray();
                        effectiveOutput = groups.Select(group => new
                        {
                    ...
                    timer.Stop();
                    completed++;
                    outputPanels.Add(CompletedPanel(panel, rawOutput, effectiveOutput, participantLane, insidePlot, pixelBounds, combinedAssembly, headerContext,
                ...
                catch (Exception exception) when (exception is not OutOfMemoryException)
                {
                    ...
                    outputPanels.Add(FailedPanel(
                        panel, rawOutput, effectiveOutput, participantLane, insidePlot, pixelBounds, combinedAssembly, headerContext,
            ...
            byte[] reportBytes = JsonSerializer.SerializeToUtf8Bytes(new
            {
                ...
                DevelopmentOnly = participantLane || insidePlot || supplemental,
    ...
    private static object CompletedPanel(
        ...
        double elapsedMilliseconds) => new
        {
            ...
            AssemblyContext = AssemblyContext(panel, participantLane, insidePlot, pixelBounds, combinedAssembly, headerContext),
    ...
    private static object? AssemblyContext(
        EvaluationPanel panel, bool participantLane, bool insidePlot, bool pixelBounds, bool combinedAssembly, bool headerContext) =>
        headerContext ? new
        {
            ...
            HeaderLayoutCompositionVersion = HeaderLayoutRoleResolver.CompositionVersion,
        ...
        bool insidePlot)
    {
        ...
        foreach (JsonElement record in records)
        {
            ...
            output.Add(new EvaluationPanel(
                ...
                crop, requestedCrop, sourceToPanel, panelToSource, plotBounds,
                insidePlot ? ReadPhaseDividerXs(reportPanel.GetProperty("axis"), plotBounds) : [],
                pngPath, pngSha,
    ...
    internal static IReadOnlyList<double> ReadPhaseDividerXs(JsonElement axis, OcrRectangle plotBounds)
    {
        JsonElement geometry = axis.GetProperty("geometry");
        if (Text(geometry, "coordinate_space") != "original_pixels")
        {
            throw new InvalidDataException("Phase divider geometry is not in panel original pixels.");
        }
        var positions = new List<double>();
        foreach (JsonElement divider in geometry.GetProperty("phase_dividers").EnumerateArray())
        {
            JsonElement midpoint = divider.GetProperty("line").GetProperty("midpoint");
            double x = midpoint.GetProperty("x").GetDouble();
            double y = midpoint.GetProperty("y").GetDouble();
            if (!midpoint.GetProperty("is_finite").GetBoolean() ||
                !double.IsFinite(x) || !double.IsFinite(y) ||
                x < plotBounds.Left || x > plotBounds.Right)
            {
                throw new InvalidDataException("Detected phase divider lies outside the plot or is nonfinite.");
            }
            positions.Add(x);
        }
        return Array.AsReadOnly(positions.Distinct().Order().ToArray());
    }
