...
internal static class Program
{
    ...
    {
        if (args.Length == 5 && args[0] == "--check-open-ocr-axis-fixture")
        {
            Console.WriteLine(JsonSerializer.Serialize(await OpenOcrArchiveFixtureCheck.RunAxisAsync(
                RepositoryRoot(), args[1], args[2], args[3], args[4]).ConfigureAwait(false), JsonOptions));
            return 0;
        }
        ...
        if (args.Length == 1 && args[0] == "--self-test-sealed-ocr-worker")
        {
            Console.WriteLine(JsonSerializer.Serialize(OriginalDbOcrSealedWorkerSelfTest.Run(), JsonOptions));
            return 0;
        }
        if (args.Length > 0 && args[0] == OriginalDbOcrSealedWorker.Command)
        {
            return await OriginalDbOcrSealedWorker.RunAsync(args, RepositoryRoot()).ConfigureAwait(false);
        }
        ...
        if (args.Length == 1 && args[0] == "--self-test-sealed-ocr-corpus")
        {
            Console.WriteLine(JsonSerializer.Serialize(
                await OriginalDbOcrInMemoryCorpusEvaluatorSelfTest.RunAsync().ConfigureAwait(false), JsonOptions));
            return 0;
        }
        ...
        if (args.Length == 1 && args[0] == "--self-test-official-head-candidate")
        {
