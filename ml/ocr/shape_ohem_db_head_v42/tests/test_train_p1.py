...
from ml.ocr.shape_ohem_db_head_v42 import train_p1 as subject


def _training_result() -> shape_ohem_head_training.TrainingResult:
    epochs = tuple(
        shape_ohem_head_training.EpochLoss(
            epoch=index,
            optimizer_steps=index * 28,
            training_step_mean_loss=0.5,
            full_train_loss=0.5,
            full_positive_shape_ohem_loss=0.5,
            full_empty_negative_loss=None,
            draw_order_sha256="e" * 64,
        )
        for index in range(1, 61)
    )
    return shape_ohem_head_training.TrainingResult(
        ...
        torch_backend=shape_ohem_head_training.TORCH_BACKEND,
        trainable_parameter_names=shape_ohem_head_training.TRAINABLE_PARAMETER_NAMES,
...
def test_actual_config_binds_bundle_and_preserves_v40_inputs() -> None:
    ...
    assert sha256_file(root / "ml/ocr/official_bakeoff/shape_ohem_head_training.py") == (
...
def test_recipe_changes_only_positive_objective_from_v40() -> None:
    ...
    assert subject._RECIPE["positive_objective"] == "equal_mean_dice_and_balanced_ohem_bce"
    assert subject._RECIPE["negative_ratio"] == 3
    assert subject._RECIPE["objective_identity"] == shape_ohem_head_training.OBJECTIVE_IDENTITY
    assert subject._default_dependencies().train_head is shape_ohem_head_training.train_shape_ohem_head
...
def test_training_result_requires_ohem_identity_and_fixed_recipe() -> None:
    ...
    with pytest.raises(subject.V42TrainingError, match="fixed OHEM recipe"):
        subject._validate_training_result(changed, 28)  # type: ignore[arg-type]

    losses = list(result.epoch_losses)
    losses[1] = shape_ohem_head_training.EpochLoss(
        **{**losses[1].__dict__, "full_train_loss": 0.25}
    )
    wrong_selection = shape_ohem_head_training.TrainingResult(
        **{**result.__dict__, "epoch_losses": tuple(losses)}
    )
    with pytest.raises(subject.V42TrainingError, match="first minimum"):
        subject._validate_training_result(wrong_selection, 28)

    negative = list(result.epoch_losses)
    negative[0] = shape_ohem_head_training.EpochLoss(
        **{**negative[0].__dict__, "training_step_mean_loss": -0.1}
    )
    with pytest.raises(subject.V42TrainingError, match="epoch evidence"):
        subject._validate_training_result(
            shape_ohem_head_training.TrainingResult(
...
def test_source_drift_rejects_before_authorization(
    ...
    with pytest.raises(subject.V42TrainingError, match="source bundle"):
...
def test_authorization_config_mismatch_rejects_before_optimizer(
    ...
    with pytest.raises(subject.V42TrainingError, match="authorization configuration"):
        subject.train_candidate(
            tmp_path / "run", Path("config.json"), repository_root=tmp_path,
            dependencies=deps,
        )
...
def test_prepare_delegates_to_exact_v40_config_without_optimization(
    ...
    objective = tmp_path / "ml/ocr/official_bakeoff/shape_ohem_head_training.py"
...
def test_success_report_and_authorization_use_only_v42_identity(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    config = {
        ...
        "base_v40_result": {"path": "base-result.json",
                            "sha256": subject.BASE_V40_RESULT_SHA256},
        "shape_ohem_training_source": {"path": "ohem.py",
                                 "sha256": subject.OHEM_TRAINING_SOURCE_SHA256},
        "base_config": {
    ...
    assert report["objective_source"] == config["shape_ohem_training_source"]
    assert report["training_result"]["negative_ratio"] == 3
    assert report["training_result"]["objective_identity"] == (
        shape_ohem_head_training.OBJECTIVE_IDENTITY
    )
