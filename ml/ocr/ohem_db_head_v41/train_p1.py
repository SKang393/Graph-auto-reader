...
"""Source-bound train-only runner for the isolated V41 OHEM objective."""
...
from ml.ocr.official_bakeoff import frozen_trunk_head, ohem_head_training


REPO_ROOT = Path(__file__).resolve().parents[3]
ROOT = Path("ml/ocr/ohem_db_head_v41")
CONFIG_PATH = ROOT / "training/p1.json"
PROTOCOL_PATH = ROOT / "protocol.json"
TASK = "ocr-detection"
REVISION = "graph-text-ohem-db-head-v41"
CANDIDATE_ID = "P1"
CONFIG_SCHEMA = "graphreader.ocr-ohem-db-head-v41-candidate-config.v1"
STAGE_SCHEMA = "graphreader.ocr-ohem-db-head-v41-training-stage.v1"
...
OHEM_TRAINING_SOURCE_SHA256 = "5b9ddd078e7c5f8467bb310268b4c14dd36b2cd0d9ae249789e51b1b7830fb63"
CHECKPOINT_NAME = "selected-head.pt"
ONNX_NAME = "detector-ohem-db-head-v41-p1.onnx"
...
_CONFIG_KEYS = {
    ...
    "ohem_training_source", "recipe", "expected_data", "synthetic_only",
    "private_data", "sealed_data", "production_approval",
}
_RECIPE = {
    "seed": ohem_head_training.SEED,
    "epochs": ohem_head_training.EPOCHS,
    "batch_size": ohem_head_training.BATCH_SIZE,
    "learning_rate": ohem_head_training.LEARNING_RATE,
    "weight_decay": ohem_head_training.WEIGHT_DECAY,
    "positive_objective": "balanced_ohem_bce_with_logits",
    "negative_ratio": ohem_head_training.NEGATIVE_RATIO,
    "objective_identity": ohem_head_training.OBJECTIVE_IDENTITY,
...
class V41TrainingError(RuntimeError):
    """The V41 authorization, dependency, objective, or output contract is invalid."""
...
class TrainingDependencies:
    ...
    train_head: Callable[..., ohem_head_training.TrainingResult]
...
def _default_dependencies() -> TrainingDependencies:
    return TrainingDependencies(
        ...
        ohem_head_training.train_ohem_head,
...
def prepare_training(
    ...
    *,
    ...
    """Authenticate V41 and reuse the exact V40 features and inverse targets."""
    ...
    if source_bundle_sha256(root, RUNNER_SOURCE_PATHS) != expected_bundle:
        raise V41TrainingError("V41 runner source bundle differs from the candidate configuration")
    ...
    _exact_descriptor(
        root, config["ohem_training_source"],
        Path("ml/ocr/official_bakeoff/ohem_head_training.py"),
    ...
    if base.config_sha256 != BASE_V40_CONFIG_SHA256:
        raise V41TrainingError("Prepared V40 configuration identity changed")
    if len(base.training_panels) != _EXPECTED_DATA["train"]["panel_count"]:
        raise V41TrainingError("Prepared V40 training panel count changed")
...
def train_candidate(
    ...
    *,
    ...
    """Run one authorized train-only V41 stage; leave actual C# dev evaluation pending."""

    root = repository_root.resolve()
    output = (output_dir if output_dir.is_absolute() else root / output_dir).resolve()
    v40._require_inside(root, output, "output")
    if output.exists():
        raise V41TrainingError(f"V41 training output already exists: {output}")
    relative_config = v40._relative(root, config_path, "configuration")
    ...
    phase = "preflight"
    result: ohem_head_training.TrainingResult | None = None
    try:
        ...
        if authorization.binding.get("candidate_config_sha256") != prepared.config_sha256:
            raise V41TrainingError(
                "Training authorization configuration differs from prepared V41 bytes")
        if authorization.snapshot_path is None:
            raise V41TrainingError("Training authorization omitted its source snapshot")
...
def _validate_training_result(
    result: ohem_head_training.TrainingResult,
    panel_count: int,
) -> None:
    fixed = (
        isinstance(result, ohem_head_training.TrainingResult)
        and result.optimizer_steps == ohem_head_training.EPOCHS * panel_count
        and result.epochs == ohem_head_training.EPOCHS
        and result.panel_count == panel_count
        and result.positive_panel_count == panel_count
        and result.empty_positive_panels == 0
        and result.seed == ohem_head_training.SEED
        and result.batch_size == ohem_head_training.BATCH_SIZE
        and result.learning_rate == ohem_head_training.LEARNING_RATE
        and result.weight_decay == ohem_head_training.WEIGHT_DECAY
        and result.negative_ratio == ohem_head_training.NEGATIVE_RATIO
        and result.objective_identity == ohem_head_training.OBJECTIVE_IDENTITY
        and result.torch_backend == ohem_head_training.TORCH_BACKEND
        and result.trainable_parameter_names == ohem_head_training.TRAINABLE_PARAMETER_NAMES
    ...
    if not fixed:
        raise V41TrainingError("V41 training result differs from the fixed OHEM recipe")
    for index, row in enumerate(result.epoch_losses, start=1):
        values = (
            row.training_step_mean_loss, row.full_train_loss,
            row.full_positive_ohem_bce_loss, row.full_empty_negative_loss,
        )
        if (
            row.epoch != index
            or row.optimizer_steps != index * panel_count
            or row.full_positive_ohem_bce_loss is None
            ...
            raise V41TrainingError("V41 epoch evidence differs from the fixed OHEM recipe")
    minimum_epoch = min(
        result.epoch_losses,
        key=lambda row: row.full_train_loss,
    ).epoch
    if result.selected_epoch != minimum_epoch:
        raise V41TrainingError("V41 checkpoint is not the first minimum full-train loss")
...
def _validate_patch(patch: frozen_trunk_head.PatchResult, onnx_path: Path) -> None:
    if (
        ...
        raise V41TrainingError("V41 ONNX did not replace exactly the nine reviewed head constants")
...
def _success_report(
    ...
    return {
        ...
        "objective_source": dict(prepared.config["ohem_training_source"]),
...
def _load_config(path: Path, root: Path) -> tuple[dict[str, Any], str]:
    config = v40._read_json_exact(path, None, "V41 configuration")
    if set(config) != _CONFIG_KEYS:
        raise V41TrainingError("V41 configuration fields changed")
    fixed = {
    ...
    if any(config.get(key) != value for key, value in fixed.items()):
        raise V41TrainingError("V41 configuration identity or scope changed")
    if config.get("recipe") != _RECIPE or config.get("expected_data") != _EXPECTED_DATA:
        raise V41TrainingError("V41 fixed recipe or full data inventory changed")
    ...
    _exact_descriptor(
        ...
        BASE_V40_RESULT_SHA256, "base V40 outcome")
    _exact_descriptor(
        root, config.get("ohem_training_source"),
        Path("ml/ocr/official_bakeoff/ohem_head_training.py"),
        OHEM_TRAINING_SOURCE_SHA256, "OHEM training source")
...
def _exact_descriptor(
    ...
    try:
        ...
        raise V41TrainingError(str(error)) from error
...
__all__ = [
    ...
    "TrainingDependencies", "V41TrainingError", "prepare_training", "train_candidate",
]
