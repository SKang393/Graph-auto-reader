...
"""Source-bound train-only runner for the isolated V43 training duration."""
...
ROOT = Path("ml/ocr/extended_db_head_v43")
CONFIG_PATH = ROOT / "training/p1.json"
PROTOCOL_PATH = ROOT / "protocol.json"
TASK = "ocr-detection"
REVISION = "graph-text-extended-db-head-v43"
CANDIDATE_ID = "P1"
CONFIG_SCHEMA = "graphreader.ocr-extended-db-head-v43-candidate-config.v1"
STAGE_SCHEMA = "graphreader.ocr-extended-db-head-v43-training-stage.v1"
...
EXTENDED_TRAINING_SOURCE_SHA256 = "348962cf0800c9c4ea1bf8bf52685159e5876c9cf062510b2ddac84389fd9328"
CHECKPOINT_NAME = "selected-head.pt"
ONNX_NAME = "detector-extended-db-head-v43-p1.onnx"
...
RUNNER_SOURCE_PATHS = tuple(dict.fromkeys((
    *v40.RUNNER_SOURCE_PATHS,
    ...
    Path("ml/ocr/shape_ohem_db_head_v42/P1_RESULT.json"),
    Path("docs/GOAL-22-OCR-TRAINING-DURATION-DIAGNOSIS.json"),
    ROOT / "__init__.py",
    ROOT / "train_p1.py",
    PROTOCOL_PATH,
)))
...
_CONFIG_KEYS = {
    ...
    "extended_training_source", "recipe", "expected_data", "synthetic_only",
    "private_data", "sealed_data", "production_approval",
}
_RECIPE = {
    "seed": extended_head_training.SEED,
    "epochs": extended_head_training.EPOCHS,
    "intraop_threads": extended_head_training.INTRAOP_THREADS,
    "batch_size": extended_head_training.BATCH_SIZE,
    "learning_rate": extended_head_training.LEARNING_RATE,
    "weight_decay": extended_head_training.WEIGHT_DECAY,
...
class V43TrainingError(RuntimeError):
    """The V43 authorization, dependency, objective, or output contract is invalid."""
...
class TrainingDependencies:
    ...
    train_head: Callable[..., extended_head_training.TrainingResult]
...
def _default_dependencies() -> TrainingDependencies:
    return TrainingDependencies(
        ...
        extended_head_training.train_extended_head,
...
def prepare_training(
    ...
    *,
    ...
    """Authenticate V43 and reuse the exact V40 features and inverse targets."""
    ...
    if source_bundle_sha256(root, RUNNER_SOURCE_PATHS) != expected_bundle:
        raise V43TrainingError("V43 runner source bundle differs from the candidate configuration")
    ...
    _exact_descriptor(
        root, config["extended_training_source"],
        Path("ml/ocr/official_bakeoff/extended_head_training.py"),
        EXTENDED_TRAINING_SOURCE_SHA256, "extended Dice training source")
    ...
    if base.config_sha256 != BASE_V40_CONFIG_SHA256:
        raise V43TrainingError("Prepared V40 configuration identity changed")
    if len(base.training_panels) != _EXPECTED_DATA["train"]["panel_count"]:
        raise V43TrainingError("Prepared V40 training panel count changed")
...
def train_candidate(
    ...
    *,
    ...
    """Run one authorized train-only V43 stage; leave actual C# dev evaluation pending."""

    root = repository_root.resolve()
    output = (output_dir if output_dir.is_absolute() else root / output_dir).resolve()
    v40._require_inside(root, output, "output")
    if output.exists():
        raise V43TrainingError(f"V43 training output already exists: {output}")
    relative_config = v40._relative(root, config_path, "configuration")
    ...
    phase = "preflight"
    result: extended_head_training.TrainingResult | None = None
    try:
        ...
        if authorization.binding.get("candidate_config_sha256") != prepared.config_sha256:
            raise V43TrainingError(
                "Training authorization configuration differs from prepared V43 bytes")
        if authorization.snapshot_path is None:
            raise V43TrainingError("Training authorization omitted its source snapshot")
...
def _validate_training_result(
    result: extended_head_training.TrainingResult,
    panel_count: int,
) -> None:
    fixed = (
        isinstance(result, extended_head_training.TrainingResult)
        and result.optimizer_steps == extended_head_training.EPOCHS * panel_count
        and result.epochs == extended_head_training.EPOCHS
        and result.intraop_threads == extended_head_training.INTRAOP_THREADS
        and result.panel_count == panel_count
        and result.positive_panel_count == panel_count
        and result.empty_positive_panels == 0
        and result.seed == extended_head_training.SEED
        and result.batch_size == extended_head_training.BATCH_SIZE
        and result.learning_rate == extended_head_training.LEARNING_RATE
        and result.weight_decay == extended_head_training.WEIGHT_DECAY
        and result.dice_epsilon == extended_head_training.DICE_EPSILON
        and result.torch_backend == extended_head_training.TORCH_BACKEND
        and result.trainable_parameter_names == extended_head_training.TRAINABLE_PARAMETER_NAMES
    ...
    if not fixed:
        raise V43TrainingError("V43 training result differs from the fixed extended Dice recipe")
    for index, row in enumerate(result.epoch_losses, start=1):
        ...
        if (
            ...
            or row.full_positive_dice_loss is None
            ...
            raise V43TrainingError("V43 epoch evidence differs from the fixed extended Dice recipe")
    minimum_epoch = min(
        result.epoch_losses,
        key=lambda row: row.full_train_loss,
    ).epoch
    if result.selected_epoch != minimum_epoch:
        raise V43TrainingError("V43 checkpoint is not the first minimum full-train loss")
...
def _validate_patch(patch: frozen_trunk_head.PatchResult, onnx_path: Path) -> None:
    if (
        ...
        raise V43TrainingError("V43 ONNX did not replace exactly the nine reviewed head constants")
...
def _success_report(
    ...
    return {
        ...
        "objective_source": dict(prepared.config["extended_training_source"]),
...
def _load_config(path: Path, root: Path) -> tuple[dict[str, Any], str]:
    config = v40._read_json_exact(path, None, "V43 configuration")
    if set(config) != _CONFIG_KEYS:
        raise V43TrainingError("V43 configuration fields changed")
    fixed = {
    ...
    if any(config.get(key) != value for key, value in fixed.items()):
        raise V43TrainingError("V43 configuration identity or scope changed")
    if config.get("recipe") != _RECIPE or config.get("expected_data") != _EXPECTED_DATA:
        raise V43TrainingError("V43 fixed recipe or full data inventory changed")
    ...
    _exact_descriptor(
        ...
        BASE_V40_RESULT_SHA256, "base V40 outcome")
    _exact_descriptor(
        root, config.get("extended_training_source"),
        Path("ml/ocr/official_bakeoff/extended_head_training.py"),
        EXTENDED_TRAINING_SOURCE_SHA256, "extended Dice training source")
...
def _exact_descriptor(
    ...
    try:
        ...
        raise V43TrainingError(str(error)) from error
...
__all__ = [
    ...
    "TrainingDependencies", "V43TrainingError", "prepare_training", "train_candidate",
]
