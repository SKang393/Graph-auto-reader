...
"""Owned crowded-context adaptation; immutable previous dev, canonical admission."""
...
from ..crowded_context_cache import SOURCE_PATHS, load_cache
...
REVISION = "marker-classifier-crowded-context-v8"
CONFIG_PATH = Path("ml/markers/classifier/crowded_context_v8/p1.json")
RUNNER_SOURCES = tuple(dict.fromkeys((*BASE_SOURCES, *(Path(p) for p in SOURCE_PATHS),
    Path("ml/markers/classifier/crowded_context_v8/runner.py"),
    Path("ml/markers/classifier/crowded_context_v8/__init__.py"),
    Path("ml/markers/classifier/crowded_context_v8/protocol.json"))))


RECIPE = {"epochs": 12, "batch_size": 128, "learning_rate": 0.0001,
          "weight_decay": 0.0001, "seed": 20260925,
...
def validate_config(config: dict):
    ...
    if config.get("source_checkpoint_sha256") != INITIALIZER_SHA256:
        raise ValueError("Registered V5 initializer changed")
