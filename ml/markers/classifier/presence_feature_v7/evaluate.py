...
def evaluate(root: Path, output: Path):
    ...
    trained = root/"artifacts/goal22-runs/classifier-presence-feature-v7/P1"
    training = load(trained/"report.json")
    receipt_path = root/"ml/markers/training-seals/marker-classifier/marker-classifier-presence-feature-v7/P1/result.json"
    receipt = load(receipt_path)
    if (sha(trained/"report.json") != receipt["report_sha256"]
            or training["revision"] != "marker-classifier-presence-feature-v7"
    ...
    report = {"schema": "graphreader.presence-feature-cascade-development.v1", "status": "diagnostic_only_unapproved",
