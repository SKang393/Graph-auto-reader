...
"""Owned structural-context adaptation; immutable previous dev, canonical admission."""
...
from ..structural_context_cache import SOURCE_PATHS, load_cache
...
REVISION = "marker-classifier-structural-context-v5"
CONFIG_PATH = Path("ml/markers/classifier/structural_context_v5/p1.json")
RUNNER_SOURCES = tuple(dict.fromkeys((*BASE_SOURCES, *(Path(p) for p in SOURCE_PATHS),
    Path("ml/markers/classifier/structural_context_v5/runner.py"),
    Path("ml/markers/classifier/structural_context_v5/__init__.py"),
    Path("ml/markers/classifier/structural_context_v5/protocol.json"))))


RECIPE = {"epochs": 12, "batch_size": 128, "learning_rate": 0.0001,
          "weight_decay": 0.0001, "seed": 20260922,
          "checkpoint_selection": "fixed_final_epoch_no_dev_selection"}
INITIALIZER_SHA256 = "55c53a57cfa9fa55384404ffa321fa3634a90580a03760631ee76c8eb862dea7"


def validate_config(config: dict):
    if any(config.get(key) != value for key, value in RECIPE.items()):
        raise ValueError("Registered classifier adaptation recipe changed")
    if config.get("source_checkpoint_sha256") != INITIALIZER_SHA256:
        raise ValueError("Registered V4 initializer changed")
    if config.get("sealed_runs_authorized") != 0 or config.get("private_reads") != 0:
        raise ValueError("This classifier adaptation permits synthetic training and development only")
