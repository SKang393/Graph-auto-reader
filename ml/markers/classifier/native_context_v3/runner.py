...
"""Train/dev-only classifier adaptation with canonical admission and CPU rest."""
...
from ..native_context_cache import SOURCE_PATHS, load_cache, sha256, write_json
...
REVISION = "marker-classifier-native-context-v3"
CONFIG_PATH = Path("ml/markers/classifier/native_context_v3/p1.json")
RUNNER_SOURCES = tuple(Path(p) for p in (*SOURCE_PATHS,
    "ml/markers/classifier/native_context_v3/runner.py",
    "ml/markers/classifier/native_context_v3/__init__.py",
    "ml/markers/classifier/native_context_v3/protocol.json",
    "ml/markers/classifier/native_context_evaluation.py",
    "ml/markers/classifier/model.py", "ml/markers/classifier/dataset.py",
    "ml/markers/classifier/metrics.py", "ml/markers/classifier/runtime_v2.py",
    "ml/markers/training_budget.py", "ml/markers/gate_seal.py",
    "ml/policy/evidence_policy.py", "ml/policy/evidence-policy.json",
    "ml/policy/acceptance-bars.json", "tools/Run-TrainingCpuBudget.ps1"))


class WorkBudget:
    def __init__(self, cancel: Path):
        def check_cancel():
            if cancel.exists():
                raise RuntimeError("Training cancelled by local cancellation file")
        self.check_cancel = check_cancel
        self.budget = TrainingCpuBudget(cancellation_check=check_cancel)
        self.cycles = 0
        self.work_seconds = self.rest_seconds = self.maximum_duty = 0.

    @contextmanager
    def work_block(self):
        self.check_cancel()
        with self.budget.work_block():
            yield
        observation = self.budget.last_observation
        self.cycles += 1
        self.work_seconds += observation.work_seconds
        self.rest_seconds += observation.rest_seconds
        self.maximum_duty = max(self.maximum_duty, observation.duty_fraction)

    def report(self) -> dict:
        return dict(cycles=self.cycles, work_seconds=self.work_seconds,
                    rest_seconds=self.rest_seconds, maximum_completed_cycle_duty=self.maximum_duty,
                    scope="Synchronous guarded work/rest average; not instantaneous per-core utilization")


def tensors(pixels: np.ndarray, rows: list[dict]) -> tuple[torch.Tensor, ...]:
    targets = np.zeros((len(rows), 9), np.float32)
    for index, row in enumerate(rows):
        if not row["artifact"]:
            labels = row["shape_target_indices"]
            targets[index, labels] = 1 / len(labels)
    return (
        torch.from_numpy(pixels), torch.from_numpy(targets),
        torch.tensor([row["fill_target_index"] for row in rows], dtype=torch.long),
        torch.tensor([row["artifact"] for row in rows], dtype=torch.float32),
        torch.tensor([not row["shape_label_ambiguous"] for row in rows], dtype=torch.bool),
    )


def loss_terms(outputs, shapes, fills, artifacts, unambiguous):
    shape, fill, artifact, embedding = outputs
    marker = artifacts < .5
    zero = shape.sum() * 0
    shape_loss = functional.cross_entropy(shape[marker], shapes[marker]) if marker.any() else zero
    fill_loss = functional.cross_entropy(fill[marker], fills[marker]) if marker.any() else zero
    per_item = functional.binary_cross_entropy_with_logits(artifact[:, 0], artifacts, reduction="none")
    class_losses = [per_item[mask].mean() for mask in (marker, ~marker) if mask.any()]
    artifact_loss = torch.stack(class_losses).mean()
    # No contradictory identity pairs for exact-pixel shape alternatives.
    identity_mask = marker & unambiguous
    embedding_loss = supervised_embedding_loss(
        embedding[identity_mask], shapes[identity_mask].argmax(1) * 3 + fills[identity_mask])
    loss = shape_loss + .75 * fill_loss + .65 * artifact_loss + .08 * embedding_loss
    return loss, (shape_loss, fill_loss, artifact_loss, embedding_loss)


def torch_outputs(model, pixels, budget, batch_size):
    values = []
    for start in range(0, len(pixels), batch_size):
        with budget.work_block(), torch.inference_mode():
            values.append(model(torch.from_numpy(pixels[start:start+batch_size])).numpy())
