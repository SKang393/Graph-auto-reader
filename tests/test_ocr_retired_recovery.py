...
def test_retired_reserve_recovers_only_prior_accounting(tmp_path, monkeypatch, confirmed):
    ...
    sealed_reserve.record_case_level_disclosure(registry,tmp_path,
        expected_registry_sha256=sha256_file(registry),set_id=metadata['set_id'],
        evidence_sha256='e'*64,disclosures=['truth'])
    ...
    for _ in range(2):
        ...
        assert fixture._authority(recovered)['read_status'] == ('confirmed' if confirmed else 'none')
